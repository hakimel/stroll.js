 ...
 */
(function(){var b=[];
var h=false;function f(){if(h){requestAnimFrame(f);for(var k=0,j=b.length;k<j;k++){b[k].update();}}}function g(n){if(!n.nodeName||/^(ul|li)$/i.test(n.nodeName)===false){return false;
}else{if(e(n)){a(n);}}var k=Array.prototype.slice.apply(n.children);var l=n.offsetHeight;for(var m=0,j=k.length;m<j;m++){k[m]._offsetTop=k[m].offsetTop;
k[m]._offsetHeight=k[m].offsetHeight;}b.push({domElement:n,update:function(s){var t=n.pageYOffset||n.scrollTop,r=t+l;if(t!==n.lastTop){n.lastTop=t;for(var p=0,o=k.length;
p<o;p++){var q=k[p];var u=q.className;if(q._offsetTop+q._offsetHeight<t){if(u.indexOf("past")===-1){q.classList.add("past");}}else{if(q._offsetTop>r){if(u.indexOf("future")===-1){q.classList.add("future");
